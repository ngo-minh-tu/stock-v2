...
- **Endpoint examples = happy path đầy đủ (login → run → portfolio → share + PDF)**: Cover 80% case tester verify. Còn 20% explore qua FastAPI auto-docs `/docs` (Swagger UI built-in). Không cần list 41 endpoint với curl từng cái — cost > benefit.
