...
| # | Drift | Resolution |
|---|---|---|
| 1 | PLAN.md row 11 ghi "poetry install" — reality Phase 0 chốt `uv sync` (User locked 2026-05-10) | ✅ README dùng `uv sync` + `uv run` xuyên suốt. PLAN.md không cần update (build-plan historical doc). |
...
- **PLAN.md row 11 "poetry install" obsolete**: chỉ là build-plan historical doc, không touch. README accurately documents reality (uv).
