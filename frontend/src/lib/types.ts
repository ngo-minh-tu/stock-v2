...
// GET /api/runs/{run_id}/results — Phase 6 backend returns ONLY {results, total}.
// Excluded list at separate endpoint /api/runs/{run_id}/excluded (Phase 6 added —
// see Phase 6 SUMMARY drift #1). Warnings live on `run.warnings` (Phase 5 RunFullSummary).
...
// Backend Phase 8 status response — KHÔNG progress_percent/current_step (DB schema không có).
// Frontend polling status enum + render generic "Đang chạy..." spinner.
...
export interface BacktestMetrics {
  ...
  // 9-26 weekly points per TAD g02 §8.6 — `week` ISO label like "2026-W18".
  roi_curve: { week: string; portfolio: number; vnindex: number }[];
  total_count: number;             // tickers backtested = scored_count latest run
  correct_count: number;
}
// Per-ticker mock heuristic results (TAD g02 §8.6).
// Field name `actual_return_3m` (no `_pct` suffix per spec). No `name` field — backend
// joins ticker only; FE TableRow can derive name via stocks-fixture lookup if needed.
...
// Backend (Phase 8) shape per TAD g02 §9.2. KHÔNG `shared_by` (single-user MVP).
// `top_mua` is the full `ScreeningResult` shape filtered rec=MUA top 10 (Phase 8
// build_share_data reuses results_service.to_result_row so SharedView TopMuaTable
// renders identical to the authenticated page).
...
// PUT /api/auth/password — change password (TAD g02 §9.5; backend schemas/auth.py).
