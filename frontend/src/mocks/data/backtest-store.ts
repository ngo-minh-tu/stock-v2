...
function computeMetrics(
  ...
  for (let i = 0; i <= points; i += 1) {
    const t = i / points;
    const date = new Date(startMs + t * (endMs - startMs));
    // ISO week label per backend Phase 8 shape (TAD g02 §8.6).
    const yr = date.getUTCFullYear();
    const wk = Math.ceil(((date.getTime() - Date.UTC(yr, 0, 1)) / 86_400_000 + 1) / 7);
    const week = `${yr}-W${String(wk).padStart(2, '0')}`;
...
function computeResults(id: number, seed: number): BacktestResultRow[] {
  ...
  return latest.computed.results.map((r) => {
    ...
    return {
      ...
      actual_return_3m: Number(actualReturn.toFixed(2)),
      predicted_price,
      actual_price,
