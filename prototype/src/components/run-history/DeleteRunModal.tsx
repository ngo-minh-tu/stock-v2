...
export function DeleteRunModal({ open, runId, onClose, onConfirm }: Props) {
  const t = useTranslations('runHistory.delete');
    ...
    >
      ...
      >
        <div className="flex items-start justify-between">
          ...
          <button type="button" onClick={onClose} className="p-1 opacity-70 hover:opacity-100" aria-label={t('close')}>
            <X size={16} aria-hidden="true" />
          </button>
        </div>

        <p className="text-sm">{t('confirm', { runId })}</p>
